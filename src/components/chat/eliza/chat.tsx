...
import { ChatMessages } from '@/components/chat/eliza/chat-messages'
import { TextareaWithActions } from '@/components/chat/eliza/textarea-with-actions'
import { USER_NAME, CHAT_SOURCE } from '@/config/constants'
...
import { assert } from '@/lib/utils' // Import assert helper
import { apiClient } from '@/lib/api-client' // Import API client
import { getOrGenerateSeed } from '@/lib/utils'
import { generateQueryRoomId } from '@/lib/utils' // Import new function
...
const Chat = () => {
  ...
  const [userEntity] = useLocalStorage<string>('UserEntity', uuidv4)
  ...
  const sendMessage = useCallback(
    (
      ...
      if (!messageText.trim() || !userEntity || !currentRoomId || !currentWorldId || inputDisabled) {
  ...
  useEffect(() => {
    ...
    console.log(`[Chat Init] Initializing. Agent: ${agent}, Entity: ${userEntity}, Room (Query-Specific): ${roomId}`)

    let historyLoaded = false
    let loadedMessagesCount = 0
    assert(agent && roomId, '[Chat Load API] Agent ID or Room ID missing before fetch')
    apiClient
      .getAgentMemories(agent as UUID, roomId as UUID)
      .then((loadedMessages) => {
    ...
    const handleMessage = (data: MessageBroadcastData) => {
      ...
      if (!data) return
      assert(typeof data.senderId === 'string', `[Chat handleMessage] Invalid senderId: ${data.senderId}`)
      assert(
