...
// import { CodeBlock } from '@/components/chat/eliza/code-block'
import { MemoizedMarkdown } from '@/components/chat/eliza/memoized-markdown'
...
export const ChatMessage = memo(function ChatMessage({
  ...
  assert(message && typeof message === 'object', `[ChatMessage Render] Invalid 'message' prop: ${typeof message}`)
  if (!message) return null
  assert(typeof message.name === 'string', `[ChatMessage Render] Invalid message.name: ${typeof message.name}`)
  assert(
  ...
  const markdownOptions = {
    ...
    overrides: {
      ...
      // },
      reference: {
        component: ({ children, index }: { children: React.ReactNode; index: number }) => {
  ...
  return (
    <div
      className={clsx(
        'w-full',
        message.name === USER_NAME && i !== 0 ? 'border-t pt-4 border-zinc-950/5 dark:border-white/5' : '',
      )}
    >
      <div
        className={clsx(
      ...
      {message.name !== USER_NAME && followUpPrompts && followUpPrompts.length > 0 && (
        <div className="mt-2">
