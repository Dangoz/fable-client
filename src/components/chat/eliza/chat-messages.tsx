...
import { ChatMessage } from '@/components/chat/eliza/chat-message'
...
export function ChatMessages({ messages, citationsMap, followUpPromptsMap, onFollowUpClick }: ChatMessagesProps) {
  assert(Array.isArray(messages), `[ChatMessages] 'messages' prop is not an array: ${typeof messages}`)
  assert(
  ...
  useEffect(() => {
    ...
    assert(
      typeof lastMessage.text === 'string' || lastMessage.text === null || lastMessage.text === undefined,
  ...
  return (
    <div className="flex flex-col gap-4">
      {messages.map((message, i) => {
        assert(message && typeof message === 'object', `[ChatMessages Map] Invalid message at index ${i}`)
        if (!message) return null
        const messageKey = message.id || message.createdAt
        assert(messageKey, `[ChatMessages Map] Message at index ${i} lacks id and createdAt for key.`)
        assert(
        ...
        const assistantIndex =
          message.name !== USER_NAME ? messages.slice(0, i + 1).filter((m) => m.name !== USER_NAME).length - 1 : -1
        ...
        return (
          <div key={messageKey} ref={i === messages.length - 1 ? messagesEndRef : undefined}>
            <ChatMessage
              ...
              followUpPrompts={message.name !== USER_NAME ? followUpPromptsMap[assistantIndex] : undefined}
