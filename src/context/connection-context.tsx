...
export const ConnectionProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (query.isSuccess && !wasConnected) {
      ...
      if (wasDisconnected) {
        toast.success('Successfully reconnected to the Eliza server.')
      }
      ...
      if (wasConnected) {
        toast.error('Connection to Eliza server lost.')
      }
      const logErrorMessage = query.error instanceof Error ? query.error.message : String(query.error)
      clientLogger.error(`Connection to Eliza server failed: ${logErrorMessage}`)
    }
  }, [query.isSuccess, query.isError, wasConnected, wasDisconnected, query.error])
  ...
  return <ConnectionContext.Provider value={{ status, error, refetch }}>{children}</ConnectionContext.Provider>
}
