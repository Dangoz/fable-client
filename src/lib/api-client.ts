  ...
  try {
    ...
    if (contentType?.includes('application/json')) {
      try {
        ...
        clientLogger.error('Response text:', text.substring(0, 500) + (text.length > 500 ? '...' : ''))
    ...
    if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
      ...
      throw new Error('NetworkError: Unable to connect to the server. Please check if the server is running.')
      ...
      throw new Error('NetworkError: Cross-origin request failed. Please check server CORS settings.')
    } else {
...
export const apiClient = {
  ...
  createGroupChat: (agentIds: string[], roomName: string, serverId: string, source: string, metadata?: any) => {
    const worldId = WorldManager.getWorldId()
    return fetcher({
