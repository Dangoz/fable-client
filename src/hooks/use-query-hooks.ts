...
import { GROUP_CHAT_SOURCE, USER_NAME } from '@/config/constants'
...
export function useStartAgent() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast(`${data?.name || 'Agent'} is now running`, {
...
export function useStopAgent() {
  ...
  return useMutation({
    ...
    onSuccess: (_, agentId) => {
      ...
      toast('The agent has been successfully stopped', {
...
export function useAgentMessages(agentId: UUID) {
  ...
  const messages = queryClient.getQueryData<ContentWithUser[]>(['messages', agentId, worldId]) || []
  ...
  const loadOlderMessages = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (response?.memories && response.memories.length > 0) {
        ...
        const timestamps: number[] = response.memories.map((msg: Memory): number => msg.createdAt ?? 0)
        ...
        mergedMessages.sort((a: ContentWithUser, b: ContentWithUser): number => (a.createdAt ?? 0) - (b.createdAt ?? 0))
...
export function useDeleteLog() {
  ...
  return useMutation({
    mutationFn: ({ agentId, logId }: { agentId: string; logId: string }) => apiClient.deleteLog(agentId, logId),
    ...
    onSuccess: (_, { agentId }) => {
      ...
      toast('The log entry has been successfully removed', {
...
export function useUpdateMemory() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast('The memory has been successfully updated', {
  ...
  return useQuery<Map<string, Room[]>>({
    ...
    queryFn: async () => {
      ...
      const worldRooms = rooms.data.filter(
        (room: Room) => room.worldId === WorldManager.getWorldId() && room.source === GROUP_CHAT_SOURCE,
      )
